...
$item_id = (isset($_GET['item_id']) && $_GET['item_id'] !== 'all') ? (int) $_GET['item_id'] : null;

$query = "SELECT h.*, i.item_name, 
          (SELECT h2.item_count FROM item_history h2 
           WHERE h2.item_id = h.item_id AND h2.created_at < h.created_at 
           ORDER BY h2.created_at DESC LIMIT 1) as prev_count
          FROM item_history h
